...
export function Sidebar({ mobileOpen = false, onNavigate }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "fixed inset-y-0 left-0 z-50 flex w-80 max-w-[86vw] flex-col border-r border-[var(--app-border-soft)] bg-[var(--app-sidebar)] shadow-2xl shadow-[var(--app-shadow)] transition-transform duration-300 ease-out sm:w-64 lg:static lg:translate-x-0 lg:shadow-none",
        mobileOpen ? "translate-x-0" : "-translate-x-full"
      )}
    >
      {/* Logo */}
      <div className="flex h-32 items-center gap-4 border-b border-[var(--app-border-soft)] px-7 sm:h-16 sm:gap-3 sm:px-5">
        ...
        <div className="flex flex-col">
          <span className="text-2xl font-semibold leading-[1.15] text-[var(--app-sidebar-text)] sm:text-sm sm:leading-none">AI Club</span>
          <span className="mt-1.5 text-sm leading-[1.4] text-[var(--app-sidebar-muted)] sm:mt-0.5 sm:text-[10px] sm:leading-none">Content Dashboard</span>
        </div>
      </div>

      {/* Nav */}
      <nav className="flex flex-1 flex-col gap-5 overflow-y-auto px-4 py-8 sm:gap-1 sm:px-3 sm:py-4">
        <p className="mb-2 px-3 text-xs font-semibold uppercase leading-[1.4] tracking-widest text-[var(--app-sidebar-muted)] sm:px-2 sm:text-[10px] sm:leading-normal">
          Workspace
        </p>
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? "bg-[var(--app-card-soft)] text-[var(--app-sidebar-text)]"
                  : "text-[var(--app-sidebar-muted)] hover:bg-[var(--app-card-soft)] hover:text-[var(--app-sidebar-text)]"
              )}
            >
              <div
                className={cn(
      ...
      {/* User footer */}
      <div className="border-t border-[var(--app-border-soft)] px-4 py-5 sm:px-3 sm:py-3">
        <div className="flex cursor-pointer items-center gap-4 rounded-2xl px-4 py-4 transition-colors hover:bg-[var(--app-card-soft)] sm:gap-3 sm:rounded-xl sm:px-3 sm:py-2.5">
          ...
          <div className="flex flex-col min-w-0">
            <span className="truncate text-lg font-medium leading-[1.3] text-[var(--app-sidebar-text)] sm:text-xs sm:leading-normal">AI Club</span>
            <span className="truncate text-sm leading-[1.3] text-[var(--app-sidebar-muted)] sm:text-[10px] sm:leading-normal">Admin</span>
          </div>
