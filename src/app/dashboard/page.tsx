            ...
            <div className="flex flex-col gap-6 sm:gap-3">
              {upcomingPosts.length === 0 ? (
                <div className="flex flex-col items-center gap-5 py-12 text-center sm:gap-3 sm:py-8">
                  ...
                  <div>
                    <p className="text-xl leading-[1.35] text-white/45 sm:text-sm sm:leading-normal sm:text-white/30">No posts scheduled</p>
                    <p className="mt-2 text-base leading-[1.5] text-white/30 sm:mt-0.5 sm:text-xs sm:leading-normal sm:text-white/20">Create a post to get started</p>
                  </div>
                  <button
                    onClick={() => setModalOpen(true)}
                    className="min-h-12 rounded-xl px-5 text-base font-semibold text-pink-400/85 transition-colors hover:text-pink-400 sm:min-h-0 sm:rounded-lg sm:px-0 sm:text-xs sm:font-normal sm:text-pink-400/70"
                  >
                    + New Post
                  </button>
                </div>
                ...
                upcomingPosts.map((post) => (
                  <div key={post.id} className="flex items-start gap-5 rounded-[1.5rem] border border-white/[0.03] bg-white/[0.018] p-6 sm:gap-3 sm:rounded-xl sm:border-white/[0.05] sm:bg-white/[0.02] sm:p-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="truncate text-lg font-semibold leading-[1.3] text-white/90 sm:text-xs sm:font-medium sm:leading-normal sm:text-white/80">{post.title}</p>
